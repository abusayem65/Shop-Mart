              ...
              @foreach ($categories as $category)
                <li class="mb-4" wire:key="category-{{ $category->id }}">
                  <label for="category-{{ $category->slug }}" class="flex items-center dark:text-gray-400 ">
                    <input id="category-{{ $category->slug }}" type="checkbox"  wire:model.live="selected_categories" class="w-4 h-4 mr-2" value="{{ $category->id }}">
              ...
              @foreach ($brands as $brand)
                <li class="mb-4" wire:key="brand-{{ $brand->id }}">
                  <label for="brand-{{ $brand->slug }}" class="flex items-center dark:text-gray-300">
                    <input type="checkbox" wire:model.live="selected_brands" class="w-4 h-4 mr-2" value="{{ $brand->id }}" id="brand-{{ $brand->slug }}">
              ...
              <div class="border border-gray-300 dark:border-gray-700">
                <div class="relative bg-gray-200">
                  <a href="/products/{{ $product->slug }}" class="">
                    <img src="{{ $product->image_url }}" alt="" class="object-cover w-full h-56 mx-auto ">
                  </a>
                </div>
                <div class="p-3 ">
